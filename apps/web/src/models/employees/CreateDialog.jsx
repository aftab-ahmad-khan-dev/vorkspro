...
function CreateDialog({
  ...
  useEffect(() => {
    ...
    const draftData =
      !isEditMode && !employee
        ? JSON.parse(localStorage.getItem("employeeDraft") || "{}")
        : {};
    ...
    setFormData({
      ...
      reportingManager:
        employee?.reportingManager || draftData.reportingManager || "",

      probationPeriodInMonths: isEditMode
        ? (employee?.probationPeriodInMonths ??
          draftData.probationPeriodInMonths ??
          0)
        : employee?.probationPeriodInMonths ||
          draftData.probationPeriodInMonths ||
          "",
  ...
  const handleChange = (name, value) => {
    ...
    if (name === "department") {
      ...
      const subStillValid = subs.some((sub) => sub._id === formData.subDepartment);
      if (!subStillValid) {
  ...
  const handleDraft = () => {
    ...
    draftPayload.dateOfBirth = formData.dateOfBirth
      ? formData.dateOfBirth.toISOString()
      : null;
    draftPayload.joinDate = formData.joinDate
      ? formData.joinDate.toISOString()
      : null;
  ...
  async function handleSubmit() {
    ...
    const url = isEditMode ? `update/${employee._id}` : `create`;
  ...
  return (
    <div className='space-y-6'>
      <div className='grid grid-cols-2 gap-4'>
        {!isEditMode && (
          <>
            ...
            <div className='col-span-2 grid grid-rows-[auto_auto_auto_auto] gap-6'>
              <div className='flex flex-col space-y-2 min-h-[4rem]'>
              ...
              <div className='flex flex-col space-y-2 min-h-[4rem]'>
                ...
                >
                  <SelectTrigger className='w-full min-h-[2.25rem] border border-[var(--border)] rounded-md p-2 text-[var(--text)] [&>span]:block [&>span]:truncate bg-white dark:bg-[var(--background)]'>
                  ...
                  <SelectContent sideOffset={4}>
                    {roles?.length > 0 ? (
                      ...
                      <SelectItem disabled value='none'>
                        No roles available. Create roles in Settings → Role &
                        Management.
                      </SelectItem>
                    )}
                  </SelectContent>
              ...
              <div className='flex flex-col space-y-2 min-h-[4rem]'>
                ...
                <div className='relative'>
                  <Input
                    ...
                    className='pr-10 bg-white dark:bg-[var(--background)]'
                  />
                  <button
                    type='button'
        ...
        <div className='flex flex-col space-y-2 bg-white dark:bg-[var(--background)] col-span-2 sm:col-span-1'>
          <label className='text-sm font-medium text-[var(--text)]'>
            Date of Birth
          </label>
          <DatePicker
          ...
          <div className='col-span-2 flex flex-col items-center justify-center py-10 text-center border border-dashed rounded-lg'>
            <span className='text-base font-medium'>
              {formData.leaveAllocation.length > 0
                ? ""
                : " No leave allocation added yet!"}
            </span>
            <span className='text-sm opacity-80'>
              {formData.leaveAllocation.length > 0
                ? " Add more leave allocation "
                : " No leave allocation added yet!"}
            </span>
        ...
        <div className='flex flex-col col-span-2 sm:col-span-1 space-y-2'>
          ...
          <Input
            ...
            onChange={(e) => handleChange("probationPeriodInMonths", e.target.value)}
          />
        </div>
        <div className='flex flex-col col-span-2 sm:col-span-1 space-y-2'>
          <label className='text-sm font-medium text-[var(--text)]'>
      ...
      <div className='flex justify-end gap-3 pt-4 border-t border-gray-200 text-wh dark:border-gray-700'>
        {!isEditMode && (
          <CustomTooltip tooltipContent='Save as Draft'>
            <Button
              onClick={handleDraft}
              variant='outline'
              className='border-button text-white'
            >
              <Save></Save>
            </Button>
          </CustomTooltip>
