...
export default function RoleForm({ role, preset, onSuccess, onClose }) {
  const isEdit = !!role;
  ...
  const getInitialForm = () => {
    ...
    if (preset) {
      return {
        ...
        modulePermissions: normalizeModulePermissions(preset.modulePermissions),
  ...
  const handleSubmit = async () => {
    ...
    // When editing, sync budget toggle with backend first
    if (isEdit && role?._id) {
      setBudgetLoading(true);
      try {
        const budgetData = await apiPatch(`role/toggle-budget/${role._id}`, { cost: toggleValue });
        if (budgetData?.isSuccess) setLocalBudget(toggleValue);
      } catch (_) {}
