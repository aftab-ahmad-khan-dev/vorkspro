...
function Sidebar({ isSidebarOpen, toggleSidebar, isMobile }) {
  ...
  // Company name for branding (workspace org)
  useEffect(() => {
    let cancelled = false;
    apiGet("config/company")
      .then((res) => {
        if (cancelled || !res?.isSuccess) return;
        const name = (res?.companyName ?? res?.data?.companyName)?.trim();
