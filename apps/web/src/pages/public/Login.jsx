...
function Field({
  ...
  return (
    <div className='login-field-wrap' style={{ marginBottom: "18px" }}>
...
export default function Login() {
  ...
  const handleSignIn = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await authApi.login({ identifier: username.trim(), password });
      if (data?.isSuccess) {
        ...
        if (typeof msg === "string" && msg.toLowerCase().includes("api key required")) {
          toast.error(
            "API key missing. Add VITE_APP_API_KEY to apps/web/.env (same as API's API_KEY) and restart the dev server.",
          );
        } else {
          toast.error(msg);
        }
  ...
  const handleResetPassword = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await authApi.resetPassword(newPassword, resetToken);
      if (data?.isSuccess) {
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 1024px) {
          .login-root { flex-direction: column !important; overflow-x: hidden !important; overflow-y: auto !important; }
          .login-left-panel { flex: 0 0 auto !important; min-height: 200px !important; padding: 32px 24px !important; }
          .login-left-inner { max-width: 360px; }
          .login-brand-title { font-size: 28px !important; }
          .login-brand-sub { font-size: 12px !important; margin-bottom: 24px !important; }
          .login-feature-pills { display: none !important; }
          .login-right-panel { padding: 24px 20px 32px !important; min-height: 0 !important; align-items: flex-start !important; }
          .login-card { max-width: 100% !important; padding: 28px 24px !important; border-radius: 20px !important; }
        }
        /* Responsive: mobile */
        @media (max-width: 640px) {
          .login-left-panel { min-height: 160px !important; padding: 24px 16px !important; }
          .login-logo-box { width: 80px !important; height: 80px !important; margin-bottom: 16px !important; border-radius: 18px !important; }
          .login-logo-box img { width: 44px !important; }
          .login-brand-title { font-size: 22px !important; }
          .login-brand-sub { font-size: 11px !important; letter-spacing: 0.12em !important; margin-bottom: 16px !important; }
          .login-right-panel { padding: 16px 12px 24px !important; }
          .login-card { padding: 22px 18px !important; border-radius: 18px !important; }
          .login-heading { font-size: 22px !important; }
          .login-subheading { font-size: 12px !important; }
          .login-field-wrap { margin-bottom: 14px !important; }
          .login-input { height: 44px !important; font-size: 16px !important; padding: 0 12px !important; }
          .login-btn { height: 44px !important; font-size: 13px !important; }
          .login-code-input { font-size: 22px !important; letter-spacing: 0.25em !important; }
        }
      `}</style>
          ...
          >
            ...
            {view === "login" && (
              <form onSubmit={handleSignIn} className='form-enter'>
                <div style={{ marginBottom: 32 }}>
                  ...
                  <h2 className='login-heading' style={styles.heading}>
                    Welcome back
                  </h2>
                  <p className='login-subheading' style={styles.subheading}>
                    Sign in with your email to open your workspace.
                  </p>
                </div>
                ...
                <Link
                  to='/register'
                  className='text-xs font-medium text-center text-blue-500 block'
                >
                  Create an account?
                </Link>
                <div className='text-xs font-medium text-right text-gray-500 block'>
            ...
            {view === "verify" && (
              <form onSubmit={handleVerifyCode} className='form-enter'>
                ...
                <div className='login-field-wrap' style={{ marginBottom: 24 }}>
