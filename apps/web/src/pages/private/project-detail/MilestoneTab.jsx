...
export default function MilestonesTab({ project, refresh, onMilestonesUpdate }) {
  ...
  const handleDragEnd = async (result) => {
    ...
    try {
      const data = await apiPatch(`milestone/change-status/${draggableId}`, { status: newStatus });
      if (!data?.isSuccess) throw new Error(data?.message || "Failed");
      toast.success("Status updated");
      setTimeline((prev) => [
        {
  ...
  const handleTimelineDragEnd = async (result) => {
    ...
    try {
      const data = await apiPatch(
        `project/change-index/${draggableId}?projectId=${project._id}`,
        { fromIndex: source.index, toIndex: destination.index }
      );
      if (!data?.isSuccess) throw new Error(data?.message || "Failed");
