...
import { ArrowLeft, Edit, Target, StickyNote } from "lucide-react";
...
export default function ProjectHeader({ project, onBack, clients, employees, onEditProject, refresh, onNavigateToTab }) {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await apiPatch(`project/change-status/${project._id}`, { status: selectedStatus });
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between md:gap-6">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <Button
            size="sm"
            variant="outline"
            className="shrink-0 h-8 gap-1.5 px-3 border-[var(--border)]"
            onClick={onBack}
          >
          ...
          <div className="min-w-0">
            <h1 className="text-xl md:text-2xl text-[var(--foreground)] font-bold truncate">
              Project Details
            </h1>
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
          {onNavigateToTab && (
            <Button
              size="sm"
              variant="default"
              className="h-8 gap-1.5 px-3 bg-emerald-600 hover:bg-emerald-700 text-white border-0 text-xs font-medium"
          ...
          <Button
            size="sm"
            variant="outline"
            className="h-8 gap-1.5 px-3 border-[var(--border)] text-xs font-medium"
