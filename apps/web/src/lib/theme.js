...
const DARK_POPOVER = "oklch(24% 0.025 277)";
...
/** Popover/dropdown: slightly elevated bg for list content */
function lightPopoverFromAccent(hex) {
  const [r, g, b] = hexToRgb(hex);
...
function applyAccentPalette(root, hex, isDark) {
  ...
  root.style.setProperty("--popover", isDark ? DARK_POPOVER : lightPopoverFromAccent(hex));
  root.style.setProperty("--popover-foreground", isDark ? "oklch(0.985 0 0)" : "oklch(0.145 0 0)");
