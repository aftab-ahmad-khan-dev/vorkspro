...
const BlockageTable = ({ blockages, search, typeFilter, severityFilter, statusFilter, onUpdate }) => {
    ...
    const handleMarkResolved = async () => {
        ...
        setLoadingResolve(selectedBlockage.id);
        try {
            const data = await apiPatch(`project/blockage/update`, { _id: selectedBlockage.id, status: "resolved" });
            if (data.isSuccess) {
    ...
    return (
        <>
            <div className="space-y-6">
                {filteredBlockages.map((blockage) => (
                    <div key={blockage.id} className="rounded-xl border border-border bg-background overflow-hidden shadow-sm">
                        ...
                        {/* Metadata Grid */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-y-4 mx-4 rounded-md p-6 bg-border/30">
            ...
            <Confirmation
                ...
                name={selectedBlockage?.title}
                title="Mark as Resolved"
                className="bg-green-100 text-green-500 dark:bg-green-400/20"
                btnClassName="bg-green-600 hover:bg-green-700 text-white"
            />
        </>
    );
};

const CreateBlockageDialog = ({ onAdd, projects, employees }) => {
    ...
    const projectMilestones = selectedProjectData?.milestones?.map((m) => ({ _id: m._id, name: m.name })) || [];
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="space-y-2">
                            ...
                            <SearchableSelect
                                ...
                                items={projects.map(p => ({ _id: p._id, name: p.name }))}
                                value={selectedProject}
                                onValueChange={(v) => {
                                    setSelectedProject(v);
                                    setFormData({ ...formData, project: v, milestone: "" });
                                }}
                            />
                        </div>
                        <div className="space-y-2">
...
export default function Blockages() {
    ...
    const fetchBlockages = async () => {
        try {
            const data = await apiGet(`project/blockage/get-all`);
            if (data.isSuccess) {
    ...
    const filteredBlockages = blockages.filter((blockage) => {
        const matchesSearch =
            blockage.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            blockage.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
            (blockage.assignedTo && blockage.assignedTo.toLowerCase().includes(searchTerm.toLowerCase())) ||
            (blockage.project && blockage.project.toLowerCase().includes(searchTerm.toLowerCase()));
        const matchesType = typeFilter === "all" || blockage.type === typeFilter;
        const matchesSeverity = severityFilter === "all" || blockage.severity === severityFilter;
        const matchesStatus = statusFilter === "all" || blockage.status === statusFilter;
        return matchesSearch && matchesType && matchesSeverity && matchesStatus;
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                ...
                {hasPermission("Blockages", "Create Records") && (
                    <CreateBlockageDialog onAdd={handleAddBlockage} projects={projects} employees={formattedEmployees} />
                )}
            </div>
