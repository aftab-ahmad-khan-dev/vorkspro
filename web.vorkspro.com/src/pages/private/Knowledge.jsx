...
function Knowledge() {
  ...
  const fetchDocumentsType = async () => {
    try {
      const data = await apiGet("document-type/get-active-list");
      if (data?.isSuccess) {
  ...
  const fetchKnowledge = async () => {
    setLoading(true);
    try {
      const data = await apiGet("knowledge/get-list");
      if (data.isSuccess) {
  ...
  const confirmDelete = async () => {
    try {
      const data = await apiDelete(`knowledge/delete/${selectedDoc._id}`);
      if (data.isSuccess) {
