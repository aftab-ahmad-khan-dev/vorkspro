  ...
  return (
    ...
    >
      ...
      <header
        ...
        style={{ borderColor: "rgba(168,154,201,0.2)", backgroundColor: "rgba(12,8,21,0.9)", backdropFilter: "blur(12px)" }}
      >
        <Link to="/" className="flex items-center gap-2 text-white/90 hover:text-white">
          <img src={logo} alt="Vorks Pro" className="h-7 w-auto brightness-0 invert opacity-90" />
      ...
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3 space-y-2">
            {ROLE_PANELS.map((r) => (
              <motion.button
                ...
                style={
                  selectedRole === r.id
                    ? { backgroundColor: ACCENT_LIGHT, border: "1px solid rgba(168,154,201,0.4)" }
                    : { border: "1px solid transparent" }
                }
              >
                {r.label}
                {selectedRole === r.id && <ChevronRight size={18} className="text-white" />}
              </motion.button>
            ))}
          </div>
          ...
          <div
            ...
            style={{ backgroundColor: "rgba(37,26,60,0.4)", borderColor: "rgba(168,154,201,0.2)" }}
          >
            <div className="flex items-center gap-2 mb-6">
              <div className="h-8 w-8 rounded-lg flex items-center justify-center text-white" style={{ backgroundColor: THEME }}>
            ...
            <ul className="space-y-1">
              {role.modules.slice(0, 8).map((m, i) => {
                const Icon = m.icon;
                return (
                  <li key={i} className="flex items-center gap-3 py-2 text-white/80 text-sm">
              ...
              {role.modules.length > 8 && (
                <li className="text-white/40 text-sm py-2">+{role.modules.length - 8} more</li>
              )}
            </ul>
          </div>
          ...
          <div className="lg:col-span-4">
            <motion.div
              ...
              style={{ backgroundColor: "rgba(37,26,60,0.5)", borderColor: "rgba(168,154,201,0.25)" }}
            >
              <div
